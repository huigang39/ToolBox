...
namespace QCP {
#else
class QCP {  // when in moc-run, make it look like a class, so we get Q_GADGET,
...
enum ExportPen {
    ...
    ,
    epAllowCosmetic
    ///< Cosmetic pens are exported normally (e.g. in PDF exports,
    ///< cosmetic pens always appear as 1 pixel on screen,
    ///< independent of viewer zoom level)
};
...
enum PlottingHint {
    ...
    ///< <tt>0x001</tt> Graph/Curve lines are drawn with a faster
    ///< method. This reduces the quality especially of the line segment
    ///<                joins, thus is most effective for pen sizes
    ///<                larger than 1. It is only used for solid line
    ///<                pens.
    ,
    phImmediateRefresh = 0x002
    ///< <tt>0x002</tt> causes an immediate repaint() instead of a soft
    ...
    ,
    phCacheLabels = 0x004  ///< <tt>0x004</tt> axis (tick) labels will be cached as
                           ///< pixmaps, increasing replot performance.
};
...
enum Interaction {
    ...
    ,
    iRangeZoom = 0x002
    ///< <tt>0x002</tt> Axis ranges are zoomable with the mouse
    ///< wheel (see \ref QCPAxisRect::setRangeZoom, \ref
    ///< QCPAxisRect::setRangeZoomAxes)
    ,
    iMultiSelect = 0x004
    ///< <tt>0x004</tt> The user can select multiple objects
    ...
    ,
    iSelectAxes = 0x010  ///< <tt>0x010</tt> Axes are selectable (or parts of them,
                         ///< see QCPAxis::setSelectableParts)
    ,
    ...
    ,
    iSelectOther = 0x080  ///< <tt>0x080</tt> All other objects are selectable (e.g. your own
...
enum SelectionRectMode {
    srmNone  ///< The selection rect is disabled, and all mouse events are
             ///< forwarded to the underlying objects, e.g. for axis range dragging
    ,
    srmZoom
    ///< When dragging the mouse, a selection rect becomes active. Upon
    ...
    srmSelect
    ///< When dragging the mouse, a selection rect becomes active. Upon
    ///< releasing, plottable data points that were within the selection
    ///< rect are selected, if the plottable's selectability setting
    ///< permits. (See  \ref dataselection "data selection mechanism"
    ///< for details.)
    ,
    srmCustom
    ///< When dragging the mouse, a selection rect becomes active. It is
    ///< the programmer's responsibility to connect according slots to
    ///< the selection rect's signals (e.g. \ref
...
public:
    ...
    */
    enum PainterMode {
        ...
        ,
        pmVectorized = 0x01  ///< <tt>0x01</tt> Mode for vectorized painting (e.g. PDF export).
                             ///< For example, this prevents some antialiasing fixes.
        ,
        pmNoCaching = 0x02
        ///< <tt>0x02</tt> Mode for all sorts of exports (e.g.
        ///< PNG, PDF,...). For example, this prevents using
        ///< cached pixmap labels
        ,
        pmNonCosmetic = 0x04
        ///< <tt>0x04</tt> Turns pen widths 0 to 1, i.e. disables cosmetic
...
public:
    ...
    */
    enum UpdatePhase {
        ...
        ,
        upLayout  ///< Final phase in which the layout system places the rects of the
                  ///< elements
    };
    ...
    */
    enum SizeConstraintRect {
        ...
        ,
        scrOuterRect  ///< Minimum/Maximum size constraints apply to outer rect, thus
                      ///< include layout element margins
    };
...
public:
    ...
    */
    enum FillOrder {
        ...
        ,
        foColumnsFirst
        ///< Columns are filled first, and a new element is wrapped
...
public:
    ...
    */
    enum TickStepStrategy {
        tssReadability  ///< A nicely readable tick step is prioritized over matching
...
public:
    ...
    void setTickOrigin( double origin );  // hides base class method but calls baseclass
...
public:
    ...
    */
    enum ScaleStrategy {
        ssNone
        ///< Modifications are not allowed, the specified tick step is
        ///< absolutely fixed. This might cause a high tick density and
        ///< overlapping labels if the axis range is zoomed out.
        ,
        ssMultiples
        ///< An integer multiple of the specified tick step is allowed.
...
public:
    ...
    */
    enum AxisType {
        ...
        atTop = 0x04  ///< <tt>0x04</tt> Axis is horizontal and on the top side of
                      ///< the axis rect
        ,
        atBottom = 0x08  ///< <tt>0x08</tt> Axis is horizontal and on the bottom side
                         ///< of the axis rect
    };
    ...
    */
    enum LabelSide {
        lsInside  ///< Tick labels will be displayed inside the axis rect and clipped
                  ///< to the inner axis rect
        ,
    ...
    */
    enum ScaleType {
        ...
        ,
        stLogarithmic
        ///< Logarithmic scaling with correspondingly transformed axis
        ///< coordinates (possibly also \ref setTicker to a \ref
...
public:
    ...
    */
    enum ScatterShape {
        ...
        ,
        ssTriangle  ///< \enumimage{ssTriangle.png} an equilateral triangle, standing
                    ///< on baseline
        ,
...
*/
template < class DataType > inline bool qcpLessThanSortKey( const DataType& a, const DataType& b ) {
    return a.sortKey() < b.sortKey();
}

template < class DataType >
class QCPDataContainer  // no QCP_LIB_DECL, template class ends up in header (cpp
                        // included below)
{
public:
...
*/
template < class DataType > QCPDataContainer< DataType >::QCPDataContainer() : mAutoSqueeze( true ), mPreallocSize( 0 ), mPreallocIteration( 0 ) {}
...
*/
template < class DataType > void QCPDataContainer< DataType >::setAutoSqueeze( bool enabled ) {
...
/*! \overload
  ...
  \see add, remove
*/
template < class DataType > void QCPDataContainer< DataType >::set( const QCPDataContainer< DataType >& data ) {
    clear();
    add( data );
}
...
/*! \overload
  ...
  \see add, remove
*/
template < class DataType > void QCPDataContainer< DataType >::set( const QVector< DataType >& data, bool alreadySorted ) {
    mData              = data;
    mPreallocSize      = 0;
...
/*! \overload
  ...
  \see set, remove
*/
template < class DataType > void QCPDataContainer< DataType >::add( const QCPDataContainer< DataType >& data ) {
    if ( data.isEmpty() )
        return;
    ...
    if ( oldSize > 0
         && !qcpLessThanSortKey< DataType >( *constBegin(),
...
/*!
  ...
  \see set, remove
*/
template < class DataType > void QCPDataContainer< DataType >::add( const QVector< DataType >& data, bool alreadySorted ) {
    if ( data.isEmpty() )
        return;
    if ( isEmpty() ) {
...
/*! \overload
  ...
  \see remove
*/
template < class DataType > void QCPDataContainer< DataType >::add( const DataType& data ) {
    if ( isEmpty() || !qcpLessThanSortKey< DataType >( data, *( constEnd() - 1 ) ) )  // quickly handle appends if new data key is
    // greater or equal to existing ones
    {
        mData.append( data );
    }
    else if ( qcpLessThanSortKey< DataType >( data, *constBegin() ) )  // quickly handle prepends using
    // preallocated space
    {
        if ( mPreallocSize < 1 )
            preallocateGrow( 1 );
...
*/
template < class DataType > void QCPDataContainer< DataType >::removeBefore( double sortKey ) {
    ...
    mPreallocSize += itEnd - it;  // don't actually delete, just add it to the preallocated
...
*/
template < class DataType > void QCPDataContainer< DataType >::removeAfter( double sortKey ) {
...
*/
template < class DataType > void QCPDataContainer< DataType >::remove( double sortKeyFrom, double sortKeyTo ) {
...
/*! \overload
  ...
  \see removeBefore, removeAfter, clear
*/
template < class DataType > void QCPDataContainer< DataType >::remove( double sortKey ) {
    ...
    if ( it != end() && it->sortKey() == sortKey ) {
        if ( it == begin() )
            ++mPreallocSize;  // don't actually delete, just add it to the preallocated
        // block (if it gets too large, squeeze will take care of
        // it)
        else
            mData.erase( it );
    }
...
*/
template < class DataType > void QCPDataContainer< DataType >::squeeze( bool preAllocation, bool postAllocation ) {
    if ( preAllocation ) {
        if ( mPreallocSize > 0 ) {
...
*/
template < class DataType > typename QCPDataContainer< DataType >::const_iterator QCPDataContainer< DataType >::findBegin( double sortKey, bool expandedRange ) const {
    if ( isEmpty() )
        return constEnd();
...
*/
template < class DataType > typename QCPDataContainer< DataType >::const_iterator QCPDataContainer< DataType >::findEnd( double sortKey, bool expandedRange ) const {
    if ( isEmpty() )
        return constEnd();
...
/*!
  ...
  \see valueRange
*/
template < class DataType > QCPRange QCPDataContainer< DataType >::keyRange( bool& foundRange, QCP::SignDomain signDomain ) {
    if ( isEmpty() ) {
        foundRange = false;
        return QCPRange();
    }
    ...
    {
        if ( DataType::sortKeyIsMainKey() )  // if DataType is sorted by main key (e.g.
        // QCPGraph, but not QCPCurve), use faster
        // algorithm by finding just first and
        // last key with non-NaN value
        {
        ...
        else  // DataType is not sorted by main key, go through all data points and
        // accordingly expand range
        {
            while ( it != itEnd ) {
                if ( !qIsNaN( it->mainValue() ) ) {
                    current = it->mainKey();
...
/*!
  ...
  \see keyRange
*/
template < class DataType > QCPRange QCPDataContainer< DataType >::valueRange( bool& foundRange, QCP::SignDomain signDomain, const QCPRange& inKeyRange ) {
    if ( isEmpty() ) {
        foundRange = false;
        return QCPRange();
    }
...
*/
template < class DataType > void QCPDataContainer< DataType >::limitIteratorsToDataRange( const_iterator& begin, const_iterator& end, const QCPDataRange& dataRange ) const {
...
*/
template < class DataType > void QCPDataContainer< DataType >::preallocateGrow( int minimumPreallocSize ) {
    if ( minimumPreallocSize <= mPreallocSize )
        return;

    int newPreallocSize = minimumPreallocSize;
    newPreallocSize += ( 1u << qBound( 4, mPreallocIteration + 4, 15 ) ) - 12;  // do 4 up to 32768-12 preallocation, doubling in each
...
*/
template < class DataType > void QCPDataContainer< DataType >::performAutoSqueeze() {
    ...
    {
        shrinkPostAllocation = postAllocSize > usedSize * 1.5;  // QVector grow strategy is 2^n for static data. Watch
        // out not to oscillate!
        shrinkPreAllocation = mPreallocSize * 10 > usedSize;
    }
    else if ( totalAlloc > 1000 )  // below 10 MiB raw data be generous with preallocated
    // memory, below 1k points don't even bother
    {
        shrinkPostAllocation = postAllocSize > usedSize * 5;
        shrinkPreAllocation  = mPreallocSize > usedSize * 1.5;  // preallocation can grow into
...
public:
    ...
    virtual double selectTest( const QPointF& pos, bool onlySelectable,
                               QVariant* details = 0 ) const Q_DECL_OVERRIDE = 0;  // actually introduced in QCPLayerable as non-pure,
...
public:
    ...
    */
    enum PositionType {
        ...
        ,
        ptViewportRatio
        ///< Static positioning given by a fraction of the viewport
        ...
        ,
        ptAxisRectRatio
        ///< Static positioning given by a fraction of the axis rect
        ///< size (see \ref setAxisRect). For example, if you call
        ///< setCoords(0, 0), the position will be at the top left
        ///< corner of the axis rect. setCoords(1, 1) will be at the
        ///< bottom right corner, setCoords(0.5, 0) will be
        ///< horizontally centered and vertically at the top of the
        ///< axis rect, etc. You can also go beyond the axis rect by
        ///< providing negative coordinates or coordinates larger
        ///< than 1.
        ,
...
public:
    ...
    */
    enum RefreshPriority {
        ...
        ,
        rpQueuedRefresh
        ///< Replots immediately, but queues the widget repaint, by
        ///< calling QWidget::update() after the replot. This way
        ///< multiple redundant widget repaints can be avoided.
        ,
        rpRefreshHint
        ///< Whether to use immediate or queued refresh depends on
        ///< whether the plotting hint \ref QCP::phImmediateRefresh is
        ///< set, see \ref setPlottingHints.
        ,
        rpQueuedReplot
        ///< Queues the entire replot for the next event loop
...
*/
template < class DataType >
QCPAbstractPlottable1D< DataType >::QCPAbstractPlottable1D( QCPAxis* keyAxis, QCPAxis* valueAxis ) : QCPAbstractPlottable( keyAxis, valueAxis ), mDataContainer( new QCPDataContainer< DataType > ) {}

template < class DataType > QCPAbstractPlottable1D< DataType >::~QCPAbstractPlottable1D() {}
...
/*!
  ...
  \seebaseclassmethod
*/
template < class DataType > QCPDataSelection QCPAbstractPlottable1D< DataType >::selectTestRect( const QRectF& rect, bool onlySelectable ) const {
    ...
    if ( DataType::sortKeyIsMainKey() )  // we can assume that data is sorted by main
    // key, so can reduce the searched key
    // interval:
    {
...
*/
template < class DataType > int QCPAbstractPlottable1D< DataType >::findBegin( double sortKey, bool expandedRange ) const {
...
*/
template < class DataType > int QCPAbstractPlottable1D< DataType >::findEnd( double sortKey, bool expandedRange ) const {
...
/*!
  ...
  \seebaseclassmethod
*/
template < class DataType > double QCPAbstractPlottable1D< DataType >::selectTest( const QPointF& pos, bool onlySelectable, QVariant* details ) const {
    if ( ( onlySelectable && mSelectable == QCP::stNone ) || mDataContainer->isEmpty() )
        return -1;
    if ( !mKeyAxis || !mValueAxis )
        return -1;
    ...
    for ( typename QCPDataContainer< DataType >::const_iterator it = begin; it != end; ++it ) {
        ...
        if ( keyRange.contains( mainKey ) && valueRange.contains( mainValue ) )  // make sure data point is inside visible range, for
        // speedup in cases where sort key isn't main key and we
...
/*!
  ...
  \see setSelection
*/
template < class DataType > void QCPAbstractPlottable1D< DataType >::getDataSegments( QList< QCPDataRange >& selectedSegments, QList< QCPDataRange >& unselectedSegments ) const {
...
*/
template < class DataType > void QCPAbstractPlottable1D< DataType >::drawPolyline( QCPPainter* painter, const QVector< QPointF >& lineData ) const {
    // if drawing solid line and not in PDF, use much faster line drawing instead
    // of polyline:
    if ( mParentPlot->plottingHints().testFlag( QCP::phFastPolylines ) && painter->pen().style() == Qt::SolidLine && !painter->modes().testFlag( QCPPainter::pmVectorized )
        ...
        const int lineDataSize = lineData.size();
        while ( i < lineDataSize ) {
            if ( qIsNaN( lineData.at( i ).y() ) || qIsNaN( lineData.at( i ).x() ) || qIsInf( lineData.at( i ).y() ) )  // NaNs create a gap in the line. Also filter
    ...
    */
    enum GradientPreset {
        ...
        gpIon  ///< Half hue spectrum from black over purple to blue and finally
               ///< green (creates banding illusion but allows more precise magnitude
               ///< estimates)
        ,
        ...
        ,
        gpJet
        ///< Hue variation similar to a spectrum, often used in numerical
...
public:
    ...
    */
    enum BracketStyle {
        ...
        ,
        bsHalfEllipse  ///< A half ellipse is drawn. The size of the ellipse is given
                       ///< by the bracket width/height properties.
        ,
...
public:
    ...
    */
    enum LineStyle {
        lsNone
        ///< data points are not connected with any lines (e.g. data only
        ///< represented with symbols according to the scatter style, see
        ///< \ref setScatterStyle)
        ,
        lsLine  ///< data points are connected by a straight line
        ,
        lsStepLeft  ///< line is drawn as steps where the step height is the value of
                    ///< the left data point
        ,
...
protected:
    ...
    QVector< QPair< QCPDataRange, QCPDataRange > > getOverlappingSegments( QVector< QCPDataRange > thisSegments, const QVector< QPointF >* thisData, QVector< QCPDataRange > otherSegments,
...
protected:
    ...
    QVector< QPointF > getOptimizedCornerPoints( int prevRegion, int currentRegion, double prevKey, double prevValue, double key, double value, double keyMin, double valueMax, double keyMax,
    ...
    void   getTraverseCornerPoints( int prevRegion, int currentRegion, double keyMin, double valueMax, double keyMax, double valueMin, QVector< QPointF >& beforeTraverse,
    ...
    */
    enum SpacingType {
        ...
        stAxisRectRatio  ///< Bar spacing is given by a fraction of the axis rect
                         ///< size
        ,
        stPlotCoords  ///< Bar spacing is in key coordinates and thus scales with the
                      ///< key axis range
    };
...
public:
    ...
    void setData( const QVector< double >& keys, const QVector< double >& minimum, const QVector< double >& lowerQuartile, const QVector< double >& median, const QVector< double >& upperQuartile,
    ...
    // non-property methods:
    void addData( const QVector< double >& keys, const QVector< double >& minimum, const QVector< double >& lowerQuartile, const QVector< double >& median, const QVector< double >& upperQuartile,
                  const QVector< double >& maximum, bool alreadySorted = false );
...
public:
    ...
    */
    ...
    void setData( const QVector< double >& keys, const QVector< double >& open, const QVector< double >& high, const QVector< double >& low, const QVector< double >& close,
    ...
    // non-property methods:
    void addData( const QVector< double >& keys, const QVector< double >& open, const QVector< double >& high, const QVector< double >& low, const QVector< double >& close,
                  bool alreadySorted = false );
...
protected:
    ...
    double ohlcSelectTest( const QPointF& pos, const QCPFinancialDataContainer::const_iterator& begin, const QCPFinancialDataContainer::const_iterator& end,
                           QCPFinancialDataContainer::const_iterator& closestDataPoint ) const;
    double candlestickSelectTest( const QPointF& pos, const QCPFinancialDataContainer::const_iterator& begin, const QCPFinancialDataContainer::const_iterator& end,
                                  QCPFinancialDataContainer::const_iterator& closestDataPoint ) const;
